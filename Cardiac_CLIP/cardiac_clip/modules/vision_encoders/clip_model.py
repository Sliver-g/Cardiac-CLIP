...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor, x_mask: torch.Tensor):
        ...
        return self.attn(x, x, x, need_weights=False, attn_mask=self.attn_mask, key_padding_mask=x_mask)[0] #记得改回False
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None):
        ...
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask) for _ in range(layers)]) #改layers-1
...
class VisualTransformer(nn.Module):
    def __init__(self, input_resolution: tuple, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
                 resolution_after: tuple):
        super().__init__()
        self.input_resolution = input_resolution
        self.output_dim = output_dim
        #self.conv1 = nn.Conv2d(in_channels=3, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False) # TODO: Conv3D
        self.conv1 = nn.Conv3d(in_channels=1, out_channels=width, kernel_size=patch_size, stride=patch_size, bias=False) # 改 128*128*128->512*8*8*8
        scale = width ** -0.5
        self.class_embedding = nn.Parameter(scale * torch.randn(width))
        #self.positional_embedding = nn.Parameter(scale * torch.randn((resolution_after // patch_size) ** 2 + 1, width))
        self.positional_embedding = nn.Parameter(scale * torch.randn((resolution_after[0] // patch_size) 
                                                                     *(resolution_after[1] // patch_size)
                                                                     *(resolution_after[2] // patch_size) + 1, width)) #改 
...
class CLIP(nn.Module):
    ...
    def forward(self, image, image_mask=None): # image [B, C, H, W] --> [B, C, H, W]
...
def build_model(name, resolution_after=(128,144,144), jit=False):
    ...
    vision_patch_size = 8 
    grid_size = (16,18,18)
    image_resolution = (vision_patch_size * grid_size[0],vision_patch_size * grid_size[1],vision_patch_size * grid_size[2])
