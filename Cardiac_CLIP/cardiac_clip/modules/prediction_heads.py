...
from cardiac_clip.modules.position_embeddings import get_2d_sincos_pos_embed,get_3d_sincos_pos_embed
from cardiac_clip.modules.vision_encoders.clip_model import Transformer, LayerNorm
...
class MIMHead(nn.Module):# 改
    def __init__(self, config):
        super().__init__()
        self.hidden_size = config["hidden_size"]
        self.patch_size = config["patch_size"]
        #self.num_patches = (config["image_size"] // config["patch_size"]) ** 2
        self.num_patches = (config["image_size"][0] // config["patch_size"]) *(config["image_size"][1] // config["patch_size"]) *(config["image_size"][2] // config["patch_size"]) 
        ...
        self.decoder_embed = nn.Linear(self.hidden_size, self.decoder_hidden_size, bias=True) #(512,384)
        ...
        decoder_pos_embed = get_3d_sincos_pos_embed(self.decoder_hidden_size, (16,18,18), True)
